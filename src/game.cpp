...
void Game::playerMoveCreature(Player* player, Creature* movingCreature, const Position& movingCreatureOrigPos, Tile* toTile)
{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(0, std::bind(&Game::playerMoveCreatureByID, this,
                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	// anty trash depotow
	if (!Position::areInRange<1, 1>(playerPos, mapToPos))
	{
...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
    ...
    if (goldIngot != 0) {
        Item* remaindItem = Item::CreateItem(ITEM_GOLD_INGOT, goldIngot);
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		int32_t HealthChange = target->getHealth();
		target->gainHealth(attacker, damage.primary.value);
		HealthChange = target->getHealth() - HealthChange;

		if (damage.primary.value > 0 && !target->isInGhostMode()) {
			std::stringstream ss;

			ss << HealthChange << (HealthChange != 1 ? " hitpoints." : " hitpoint.");
			...
			strHealthChange << "+" << damage.primary.value;
			addAnimatedText(strHealthChange.str(), targetPos, TEXTCOLOR_LIGHTGREEN);
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
        ...
        addAnimatedText(strManaChange.str(), target->getPosition(), TEXTCOLOR_MAYABLUE);
